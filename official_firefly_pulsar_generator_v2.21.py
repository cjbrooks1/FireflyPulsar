...
def eval_overlap(X,Y,periodstep, pulsestep):
    """
    ...
    big_both_off=False
    #dt=pulsestep*periodstep
    dt=periodstep
    dt=np.min([X.period,Y.period])/100
    # dt=1
    Xmin = np.min([X.period,Y.period]) == X.period # Tracks if X has the smaller period
    small_list=[]
    maxphase=[]
    if Xmin:
        for i in np.arange(0,X.period+dt,dt):
            new_small=X.shift(i)
            small_list.append(new_small)
    else:
        for i in np.arange(0,Y.period+dt,dt):
            new_small=Y.shift(i)
            small_list.append(new_small)
    ...
    for f in small_list:     
        if Xmin:
            ...
            if len(Y.off_times)<=0:
                ...
                small_times= np.arange(0,((3*Y.period)),f.period)
                big_times= np.arange(0,((3*Y.period)),Y.period)
            else:
                small_times= np.arange(0,((3*Y.period)+Y.off_times[0]),f.period)
                big_times= np.arange(0,((3*Y.period)+Y.off_times[0]),Y.period)

            if len(f.on_times)<=0 and len(f.off_times)>0:
            ...
            if small_always_off == True and small_both_off == False:
                small_off_times=f.off_times[0]+small_times  
                small_on_times=np.array(['n'])

            if small_always_on == True:
                small_off_times=np.array(['n'])
                small_on_times = f.on_times[0]+small_times
            ...
            if len(X.off_times) <=0:
                small_times= np.arange(0,((3*X.period)),f.period)
                big_times= np.arange(0,((3*X.period)),X.period)
            else:
                small_times= np.arange(0,((3*X.period)+X.off_times[0]),f.period)
        ...
        #print(toi_list)
        # print(change_events)
        maxsub= np.max(sub_strings)
        maxphase.append(maxsub)


    answer = np.max(maxphase) / np.max([X.period,Y.period])
    if answer >1:
        #print(np.max(maxphase), 3*np.max([X.period,Y.period]), X, Y)
        #print('error, answer greater than 1')
        answer = 1
    return(answer)
...
def signal_cost(X,background,ws,we,periodstep,pulsestep):
    """"
    ...
    for i in background:
        ...
        similarity+= eval_overlap(X,i,periodstep,pulsestep)  ### since eval_overlap gives same value as overlap_cost but faster
        #print(similarity)
...
def process_combination(params):
    ...
    """
    periodz, pulse_fract, background, ws, we, periodstep, pulsestep = params
    ...
    prospect_cost = signal_cost(generated, background, ws, we, periodstep, pulsestep)
...
def exp_signal_modeller(background, ws, we,min_period, large_period, periodstep, pulsestep,pulsemin,pulsemax):
    ...
    combinations = [(periodz, pulse_fract, background, ws, we, periodstep, pulsestep) for periodz in float_array for pulse_fract in fraction_array]
...
def contour_map(sorted_cs,min_period, large_period, periodstep,pulsestep,absmin,absmax,weightname):
    ...
    num_levels = 15 ## 10 is chosen to determine number of contour levels.
    ...
    cbar=plt.colorbar()
    cbar.set_label('Cost',fontsize=20)
    cbar.ax.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
    cbar.ax.tick_params(labelsize=20)

    plt.xlabel('Absolute Duty Cycle',fontsize=24, fontweight='bold')
    plt.ylabel('Pulse Periods (ms)',fontsize=24, fontweight='bold')
    ...
    plt.yticks(np.round(np.arange(0, large_period + periodstep, 250),2))
    plt.xticks(np.round(np.arange(0, absmax,0.02),2))
    plt.ylim(25,2500)
...
if __name__ == "__main__":
    ...
    if operator == 'optimized pulsar':
        ...
        if howmanychamps == 'yes':
            ...
            raw = input("Enter the similarity weights (values between 0 and 1 separated by a comma): ")
        ...
        for i in range(backnumber):
            ...
            if len(background[i].on_times)==0:
                continue
            else:
        ...
        # Plot the background signalsoptimizedpulsars
        h=0
        ...
        for i in range(len(wslist)):
            ...
            for x in background:
                ...
                pcost=signal_cost(x,expback,wslist[i],1.0-wslist[i],periodstep,pulsestep)
                data.append(pcost)
            ...
            for r in data:
                if r<=x_value:
                    print(f"Background pulsar cost {r:.2f} is lower than artificial signal cost {x_value:.2f}")
